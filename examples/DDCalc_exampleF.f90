...
PROGRAM DDCalc_exampleF
  ...
  mDM = 3.0d0                               ! DM Mass in GeV.
  DM_spin = 0.5d0               ! DM Spin
  CALL DDCalc_SetWIMP_NREFT_CPT(WIMP, mDM, DM_spin)
  ...
  CALL DDCalc_SetNRCoefficient(WIMP, 11, 0, 1.5d-4)
    ! This sets the proton part of O_11 to 1.5e-4 GeV^(-2).
  CALL DDCalc_SetNRCoefficient(WIMP, 11, 1, 3.5d-4)
    ! This sets the neutron part of O_11 to 3.5e-4 GeV^(-2).
  CALL DDCalc_CalcRates(Detector,WIMP,Halo) ! This performs the actual calculation of the rates.

  WRITE (*,*) '******************************************************************'
  WRITE (*,*) 'Example 4: CRESST (2017) analysis, as before but allowing for '
  WRITE (*,*) '           poles in the non-relativistic operator Wilson '
  WRITE (*,*) '           coefficients.'
  WRITE (*,*) 'mDM       = ', mDM, ' GeV'
  WRITE (*,*) 'DM spin   = ', DM_spin
  WRITE (*,*) '               Expected signal    Observed    Expected background'
